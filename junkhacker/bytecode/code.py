...
# TODO Change the name to .bytecodeutils or something
...
LOAD_NAME = 101
LOAD_ATTR = 106
LOAD_OPCODES_CEPT_CONST = (LOAD_NAME, LOAD_ATTR)
...
STORE_FAST  = 125
STORE_DEREF = 137
STORE_OPCODES = (STORE_SLICE, STORE_MAP, STORE_SUBSCR, STORE_NAME, STORE_ATTR, STORE_GLOBAL, STORE_FAST, STORE_DEREF)
...
BAD_IMPORTS = ('xml','md5','sha1','xml.etree.ElementTree','xml.dom','xml.dom.minidom','xml.dom.pulldom','xml.sax','xml.parsers.expat')

# figure out how to access bytecode from line number or offset
class BytecodeObject(object):
  ...
  def __init__(self, pyc_file, lazy_load=True):
    ...
    self.bytecode = []
    #######
    # This was for PSSA, not used any more
    self.rets = []
    self.loads = []
    self.stores = []
    self.compares = []
...
# add bytecode to each bb here maybe?
  def build_representation(self):
    ...
    # logger.debug("self.main_module is %s\n", str(dir(self.main_module)))
    self.all_decls.append(self.main_module)
    ...
    for tpl_indices in interest_indices:
      ...
      if tpl_indices in class_decl_indices:
        ...
        decl.formal_parameters = BytecodeObject.get_formal_params(decl_co)

      decl.bytecode = self.bytecode[start_index:end_index]
      decl.lines = lines_tuple[:2]

      self.all_decls.append(decl)
  ...
  def load_bytecode(self, code_object):
    BytecodeObject.parse_code_object(code_object, self.bytecode, self.loads, self.stores, self.compares, self.rets)
    # logger.debug("Bytecode for %s:\n%s" % (code_object, show_bytecode(self.bytecode)))


  @staticmethod
  # def get_parsed_code(code_object):
  def get_parsed_code(code_object, bytecode):
    # bytecode = []
    loads = []
    stores = []
    compares = []
    rets = []
    BytecodeObject.parse_code_object(code_object, bytecode, loads, stores, compares, rets)
    return bytecode


  @staticmethod
  def parse_code_object(code_object, bytecode, loads, stores, compares, rets):
    """
...
# SSA
    logger.debug('code_object.co_varnames')
    logger.debug(code_object.co_varnames)
    logger.debug('code_object.co_names')
    logger.debug(code_object.co_names)
    logger.debug('code_object.co_consts')
    logger.debug(code_object.co_consts)
    logger.debug('code_object.co_freevars')
    logger.debug(code_object.co_freevars)
    logger.debug('code_object.co_cellvars')
    logger.debug(code_object.co_cellvars)
# SSA
    ...
    while i < length:
      ...
      #######
      # While we're at it find
      # RETURN_VALUE
      # COMPARE_OP
      # STORE_NAME
      # LOAD_NAME
      # STORE_*
      # LOAD_* (except LOAD_CONST)
      #######
      if  op == RETURN_VALUE:
        rets.append((current_index, lineno, op, arg1, cflow_in, code_object))
        # get the BB this is in
        # get the parent of the BB this is in
      if  op == COMPARE_OP:
        compares.append((current_index, lineno, op, arg1, cflow_in, code_object))
        # get the condition
      if  op in LOAD_OPCODES_CEPT_CONST:
        loads.append((current_index, lineno, op, arg1, cflow_in, code_object))
        # Uses!
      if  op in STORE_OPCODES:
        stores.append((current_index, lineno, op, arg1, cflow_in, code_object))
        # Definitions!
      #######
      ...
      if arg1 and isinstance(arg1, types.CodeType):
        # logger.debug('bout2fail')
        # BytecodeObject.parse_code_object(arg1, bytecode)
        BytecodeObject.get_parsed_code(arg1, bytecode)
        # BytecodeObject.load_bytecode(arg1, bytecode)
        # logger.debug('bout2fail')
  ...
  def get_imports_from_bytecode(code_object, bytecode):
    ...
    for import_stmt in import_stmts:
      logger.debug('import_stmt is')
      logger.debug(import_stmt)
      arg = import_stmt[0][3]
    ...
    logger.debug("final_import_stmts is")
    logger.debug(final_import_stmts)
    for imp in final_import_stmts:
      logger.debug('imp_stmt.aliases is')
      logger.debug(imp.aliases)
      for (alias,_) in imp.aliases:#  final_import_stmts:
        if alias in BAD_IMPORTS:
          logger.debug('UH OH SPAGHETTIOS')
          with open("test.txt", "a") as results:
            results.write("U text")
          # RETURN_VALUE SHOULD BE THE BAD_IMPORT
          # RETURN_VALUE SHOULD BE THE BAD_IMPORT
          # RETURN_VALUE SHOULD BE THE BAD_IMPORT
        logger.debug('alias is')
        logger.debug(alias)
      logger.debug('imp_stmt.root is')
      logger.debug(imp.root)
        # Yay :D Done already
