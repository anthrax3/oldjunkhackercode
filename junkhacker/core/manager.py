...
from ..analysis import ControlFlow, BasicBlock
from ..analysis.graph.traversals import bfs_set_buddies
...
class JunkHackerManager():
    ...
    def __init__(self, config, agg_type, debug=False, verbose=False,
        ...
        :type config: junkhacker.core.JunkHackerConfig
        :param agg_type: aggregation type (by vuln or file)
        ...
        self.files_list = []
        self.excluded_files = []
        # self.ma = meta_ast.JunkHackerMetaAst()
        self.skipped = []
        self.results = []
        self.baseline = []
        self.agg_type = agg_type
        self.metrics = metrics.Metrics()
        # self.ts = test_set.JunkHackerTestSet(config, profile)

        # set the increment of after how many files to show progress
        self.progress = constants.progress_increment
        self.scores = []
        self.file_summaries = {}
    ...
    def populate_baseline(self, data):
        ...
        run of junkhacker. Later this baseline can be used to filter out the result
    ...
    def results_count(self, sev_filter=constants.LOW,
                      conf_filter=constants.LOW):
    ...
    def output_results(self, lines, sev_level, conf_level, output_file,
        ...
        :return: -
        '''
        logger.debug("Fill me in")
        # logger.debug("HERE self.scores is %s", self.scores)
        # logger.error("HERE self.file_summaries is %s", json.dumps(self.file_summaries, indent=4, sort_keys=True))
        different_dict = {}
        for file in self.file_summaries.keys():
            no_dupes = set()
            different_dict[file] = []
            logger.error('File is %s', file)
            for vuln in self.file_summaries[file]:
                if str(vuln) not in no_dupes:
                    no_dupes.add(str(vuln))
                    different_dict[file].append(vuln)
                    # logger.error("\t\tVuln is %s", json.dumps(vuln, indent=4, sort_keys=True))

        # logger.error("different_dict is %s", json.dumps(different_dict, indent=4, sort_keys=True))
        print "[dupes] self.file_summaries is %s", json.dumps(self.file_summaries, indent=4, sort_keys=True)
        print "[no dupes]different_dict is "+json.dumps(different_dict, indent=4, sort_keys=True)

        # logger.debug("self.metrics.data is %s", json.dumps(self.metrics.data, indent=4, sort_keys=True))

        # Just print out the file and it's score, for now


        # logger.debug("self.scores is %s", self.scores)
    ...
    def discover_files(self, targets, recursive=False, excluded_paths=''):
        ...
        excluded_path_strings = self.conf.get_option('exclude_dirs') or []
        included_globs = self.conf.get_option('include') or ['*.py']
    ...
    # This just runs the AST shit on every file :/
    def run_tests(self):
        ...
        for count, fname in enumerate(self.files_list):
            logger.error("working on file : %s", fname)
            ...
            try:
                with open(fname, 'rb') as fdata:
                    try:
                        ...
                        score = self._execute_interpreter(fname, source_code,
                                                          nosec_lines)
                        self.scores.append(score)
                        self.file_summaries[fname] = score
                        ...
                        logger.error(
                            ...
                            "{0}. Run \"junkhacker --debug {0}\" to see the full "
    ...
    def _execute_interpreter(self, fname, source_code, nosec_lines):
        '''Execute taint propagation on each file

        :param fname: The name of the file being parsed
        :param source_code: Original file contents
        :param nosec_lines: The lines of code to skip
        :return: The accumulated test score
        '''
        score = []

        ## Here is where we put the code to test a file
        logger.warn("fname is %s", fname)
        # logger.warn("source_code is %s", source_code)
        # logger.warn("nosec_lines is %s", nosec_lines)

        bytecode_object = BytecodeObject('<string>')
        compiled_code = utils.get_co(source_code)
        bytecode_object.parse_code(compiled_code)

        for decl in bytecode_object.declarations:
            if decl.kind == Declaration.METHOD:
                logger.warn('decl.formal_parameters is')
                logger.warn(decl.formal_parameters)

                cflow = ControlFlow(decl)
                graph = cflow.graph

                # We need to initialize the buddy system!
                # Start new code
                logger.debug("Uh what, # of nodes is %s", len(graph.nodes))
                logger.debug("IN MANAGER Here they are %s", graph.nodes)
                ...
                if decl.kind == Declaration.METHOD:
                    ...
                    cflow.taint_propagation(cflow.root, decl, tainted=set(decl.formal_parameters), stack=[], environment={}, immune={}, previous_edges=set(), new_edge='', other_decls=None, interprocedural_mode=False, stack_of_buddies=[], score=score)
                else:
                    cflow.taint_propagation(cflow.root, current_decl=None, tainted=set(), stack=[], environment={}, immune={}, previous_edges=set(), new_edge='', other_decls=None, interprocedural_mode=False, stack_of_buddies=[], score=score)

        # res = node_visitor.JunkHackerNodeVisitor(fname, self.ma,
        #                                        self.ts, self.warn,
        #                                        nosec_lines, self.metrics)

        # score = res.process(source_code)
        # self.results.extend(res.tester.results)
        logger.error('first time score is now %s', score)
        logger.error('first time type(score) is now %s', type(score))

        # Lol removing duplicate dict's from a list like a pro:
        # for x in score:
        #     for y in score:
        #         if x == y:
        #             score.remove(y)
        # logger.error('AFTER score is now %s', score)
...
def _find_candidate_matches(unmatched_issues, results_list):
    ...
    which used to have two.  JunkHacker can't tell which of the command injection
    ...
    :param results_list: Master list of current JunkHacker findings
